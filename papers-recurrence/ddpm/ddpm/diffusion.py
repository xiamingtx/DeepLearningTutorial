...
class GaussianDiffusion(nn.Module):
    ...
    # 推理过程 remove_noise & sample  从X_T -> X_0
    @torch.no_grad()
    ...
    def sample(self, batch_size, device, y=None, use_ema=True):
        ...
        x = torch.randn(batch_size, self.img_channels, *self.img_size, device=device)  # X_T
        
        for t in range(self.num_timesteps - 1, -1, -1):
            t_batch = torch.tensor([t], device=device).repeat(batch_size)
            x = self.remove_noise(x, t_batch, y, use_ema)  # x_t -> x_{t - 1}
    ...
    # 训练过程 get_losses + forward
    def get_losses(self, x, t, y):
        noise = torch.randn_like(x)  # 生成符合高斯分布的噪声z

        perturbed_x = self.perturb_x(x, t, noise)  # x0 -> xt  这里就是得到xt
        estimated_noise = self.model(perturbed_x, t, y)  # 通过Unet得到预测的noise

        # 使用预测的noise和实际的noise计算loss
